...
class Maddpg():
    ...
    def maddpg_learn(self, experiences, own_idx, other_idx, gamma=GAMMA):
        ...
        all_next_actions = torch.cat((agent.actor_target(own_states).detach(), 
                                      self.agents[other_idx].actor_target(other_states).detach()),
                                     dim =1).to(device)
        with torch.no_grad():
        ...
        # ---------------------------- update actor ---------------------------- #
        # Compute actor loss
        all_actions_pred = torch.cat((agent.actor_local(own_states), 
                                      self.agents[other_idx].actor_local(other_states).detach()),
                                     dim = 1).to(device) 
...
def decode(size, num_agents, id_agent, sa):
    
    list_indices  = torch.tensor([ idx for idx in 
                            range(id_agent * size, id_agent * size + size) ]).to(device)    
